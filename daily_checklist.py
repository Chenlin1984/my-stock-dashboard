...
def evaluate_market_status_v4_final(current_price: float, ma_240: float,
    ...
    margin_maintenance_ratio = margin_maintenance_ratio or 170.0
    ...
    is_margin_danger = margin_maintenance_ratio < 160.0
    ...
    if is_bull_market:
        ...
        if is_margin_danger:
            signal = "🔴 恐慌空頭 (斷頭邊緣)"
            action = "趨勢偏空且融資斷頭潮湧現。嚴格控制現金水位，基金部位暫停扣款或靜待落底。"
            hold_ratio = "0% - 20%"
        else:
            signal = "🔴 空頭防禦"
    ...
    return {
        ...
        "Is_Margin_Danger": is_margin_danger,
