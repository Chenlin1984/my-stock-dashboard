    ...
    if do_refresh or 'cl_data' not in st.session_state:
        ...
        if True:  # noqa
            ...
            def _job_macro():
                ...
                # 4. NDC 景氣對策信號
                # 主站 index.ndc.gov.tw 為 Angular SPA，改用政府開放資料平臺 (data.gov.tw)
                # 備援：OECD CLI（已在步驟3取得）映射為景氣燈號近似分數
                _ndc_fetched = False
                try:
                    ...
                    for _ng_res in [
                        ...
                        try:
                            ...
                            print(f'[Macro/NDC/Gov] ❌ {_ng_res[:8]}: {_nge}')
                except Exception as _ngoe:
    ...
    try:
        ...
        if _rp:
            ...
            try:
                ...
                _proxy_rp = _m_rp.group(1) if _m_rp else 'N/A'
            except Exception:
                _proxy_rp = 'N/A'
            ...
            def _rp_ts(_df):
                ...
                for _c in _df.columns:
                    _cl2 = str(_c).lower()
                    _fmt2 = '%Y%m%d' if _cl2 == '_date' else None
                    if _cl2 in ('_date', 'date', 'datetime', 'timestamp', '日期', 'quarter', 'period'):
                        try:
            ...
            if _e1rp.get('cur_yield') is not None:
                _rp[f'{_epfxrp} | 殖利率與技術分析'] = {'last_updated': _proxy_rp, 'rows': 1, 'category': 'ETF', 'frequency': 'daily'}
            else:
                _rp[f'{_epfxrp} | 殖利率與技術分析'] = {'last_updated': 'N/A', 'rows': 0, 'category': 'ETF', 'frequency': 'daily', 'missing': True}
