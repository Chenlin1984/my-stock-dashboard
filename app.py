...
<div style="font-size:11px;color:#484f58;">每次操作前先看這裡，5分鐘掌握今日全局</div>
    ...
    if do_refresh or 'cl_data' not in st.session_state:
        ...
        if True:  # noqa
            ...
            def _job_macro():
                """總經拼圖 v5.0：VIX/CPI/PMI/NDC/Export 並行抓取（省去封鎖的 FRED PMI ~20s）"""
                import requests as _rq_mc, pandas as _pd_mc, io as _io_mc
                from concurrent.futures import ThreadPoolExecutor as _TPE, as_completed as _asc_mc

                def _mk_s():
                    try:
                        from tw_stock_data_fetcher import build_proxy_session as _bps
                        return _bps()
                    except Exception:
                ...
                def _fetch_vix():
                    ...
                    try:
                        ...
                        for _host in ('query1.finance.yahoo.com', 'query2.finance.yahoo.com'):
                            try:
                                _rv = _s.get(f'https://{_host}/v8/finance/chart/%5EVIX',
                                             params={'range': '3mo', 'interval': '1d'},
                                             headers=_hdrs, timeout=12, verify=False)
                                print(f'[Macro/VIX] {_host} status={_rv.status_code}')
                                if _rv.status_code != 200: continue
                                _j = _rv.json()
                                _rs = (_j.get('chart') or {}).get('result') or []
                                if not _rs: continue
                                ...
                                if len(_ok) >= 3:
                                    _vd = [p[0] for p in _ok]; _vv = [p[1] for p in _ok]
                ...
                # ── 4. NDC 景氣對策信號 ────────────────────────────────────────────
                # bare requests.get()；msg=='success'；start_date='2023-01-01'
                # 回傳格式維持 {'ndc_signal': {'score', 'signal', 'date'}}
                def _fetch_ndc():
                    import requests as _rq_n, pandas as _pd6
                    ...
                    try:
                        _r_n = _rq_n.get(
                            'https://api.finmindtrade.com/api/v4/data',
                            params=_p_n, timeout=8,
                            headers={'Authorization': f'Bearer {_fm_tok_n}'} if _fm_tok_n else {})
                        _j_n = _r_n.json()
                        _rows_n = _j_n.get('data') or []
                        print(f'[Macro/NDC/FM] msg={_j_n.get("msg")} rows={len(_rows_n)}')
                        if _j_n.get('msg') == 'success' and _rows_n:
                ...
                # ── 5. 台灣出口 YoY ────────────────────────────────────────────────
                # bare requests.get()；msg=='success'；start_date='2022-01-01'；iloc[-13] YoY
                ...
                def _fetch_export():
                    ...
                    try:
                        _r_ex = _rq_ex.get(
                            'https://api.finmindtrade.com/api/v4/data',
                            params=_p_ex, headers=_hdrs7, timeout=8)
                        _j_ex = _r_ex.json()
                        _rows_ex = _j_ex.get('data') or []
                        print(f'[Macro/Export/FM] msg={_j_ex.get("msg")} rows={len(_rows_ex)}')
                        if _j_ex.get('msg') == 'success' and _rows_ex:
                            ...
                            if 'value' in _df_ex.columns and 'date' in _df_ex.columns:
                                ...
                                if len(_df_ex) >= 13:
                                    _cur = _df_ex['_v'].iloc[-1]
                                    _prev = _df_ex['_v'].iloc[-13]
                                    _yoy_ex = round((_cur / _prev - 1) * 100, 2) if _prev else None
                                    if _yoy_ex is not None:
                                        _date_ex = str(_df_ex['date'].iloc[-1])[:7]
                                        print(f'[Macro/Export/FM] ✅ YoY={_yoy_ex:.2f}% date={_date_ex}')
                                        return {'tw_export': {'yoy': _yoy_ex, 'date': _date_ex, 'source': 'FinMind-ME'}}
                                else:
                                    print(f'[Macro/Export/FM] ⚠️ 資料不足 13 列，got={len(_df_ex)}')
                ...
                _r = {}
                with _TPE(max_workers=5) as _pool_mc:
                    _futs_mc = {
...
• TAIFEX 可達時自動補充：前五大/前十大/精確PCR/未平倉/韭菜精確值<br>
    ...
    if _ai_exp is not None:
        ...
        _cli_str = (f'OECD CLI={_ai_cli:.2f}' if _ai_cli is not None else
                    f'ISM PMI={_ai_pmi:.1f}' if _ai_pmi is not None else '')
        ...
        _cli_str = f'OECD CLI={_ai_cli:.2f}' if _ai_cli is not None else f'ISM PMI={_ai_pmi:.1f}'
...
【語言規範】統一使用繁體中文。禁止出現「一定要買」、「保證獲利」等字眼。數據精確到小數點後兩位。"""
            ...
            with st.spinner('🤖 AI 首席顧問分析中（約30秒）...'):
                _ai_sum_result = gemini_call(_ai_sum_prompt, max_tokens=1800)
